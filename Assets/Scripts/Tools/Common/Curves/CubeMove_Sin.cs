...
{
    //[ExecuteInEditMode]
    public class CubeMove_Sin : MiBaseMonoBeHaviourClass
    {
        ...
        private void Launch()
        {
            ...
            //前一个点
            posXT2 = posX;
            posYT2 = Mathf.Sin(segments * Mathf.PI * (gameTime + minnumNext)) * height;
            posZT2 = segments * Mathf.PI * (gameTime + minnumNext) * segmentsWidth;

            //后一个点
            posXT3 = posX;
            posYT3 = Mathf.Sin(segments * Mathf.PI * (gameTime - minnumPrior)) * height;
            ...
            //Debug.Log($"{character.transform.TransformPoint(new Vector3(0, 0, 1)).normalized}    {character.transform.TransformDirection(new Vector3(0, 0, 1)).normalized}    {character.transform.TransformVector(new Vector3(0, 0, 1)).normalized}   {dirction}    {dir}");
            //赋值 角度
            ...
            for (float i = 0; i <= 1; i += interval)
            {
                ...
                posXT1 = posX;
                posYT1 = Mathf.Sin(segments * Mathf.PI * i) * height;
                posZT1 = segments * Mathf.PI * i * segmentsWidth;

                posXT2 = posX;
                posYT2 = Mathf.Sin(segments * Mathf.PI * (i + interval)) * height;
                ...
                posXT3 = posX;
                posYT3 = Mathf.Sin(segments * Mathf.PI * (i - interval)) * height;
